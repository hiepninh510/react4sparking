import { IBackendRes, ICompany, IAccount, IUser, IModelPaginate, IGetAccount, IJob, IResume, IPermission, IRole, ISubscribers, IProject } from '@/types/backend';
...
Module Project
 */
export const callCreateProject = (name: string, address: string, description: string, logo: string) => {
    return axios.post<IBackendRes<IProject>>('/api/v1/projects', { name, address, description, logo })
}

export const callUpdateProject = (id: string, name: string, address: string, description: string, logo: string) => {
    return axios.patch<IBackendRes<IProject>>(`/api/v1/projects/${id}`, { name, address, description, logo })
}

export const callDeleteProject = (id: string) => {
    return axios.delete<IBackendRes<IProject>>(`/api/v1/projects/${id}`);
}

export const callFetchProject = (query: string) => {
    return axios.get<IBackendRes<IModelPaginate<IProject>>>(`/api/v1/projects?${query}`);
}

export const callFetchProjectById = (id: string) => {
