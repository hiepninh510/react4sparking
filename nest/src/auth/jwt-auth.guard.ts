...
export class JwtAuthGuard extends AuthGuard('jwt') {
    ...
    handleRequest(err, user, info, context: ExecutionContext) {
        ...
        if (err || !user) {
            throw err || new UnauthorizedException('Token khong hop lệ/ khong co Bearer Token o Header');
        }
        ...
        if (!isExist && !isSkipPermission)
            throw new ForbiddenException('Ban khong co quyen truy cap dia chi nay');
